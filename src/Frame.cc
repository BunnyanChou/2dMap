     ...
     mTimeStamp(timeStamp), mK(K.clone()),mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth), gps(vGps), mTgps_from_w(Tgps_from_w)
{
    // Frame ID
    mnId=nNextId++;
    mTw_from_gps = cv::Mat::eye(4, 4, CV_32F);
    if (mTgps_from_w.empty()) {
    ...
    mTgpsc = cv::Mat::eye(4, 4, CV_32F);
    for (int i = 0; i < 3; ++i) {
        mTgpsc.at<float>(i, 3) = static_cast<float>(vGps[i]);
    }
    ...
    if(mvKeys.empty()) {
        std::cout << "No keypoint detected." << std::endl;
        return;
    }
