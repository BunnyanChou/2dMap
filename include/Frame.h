...
public:
    ...
    // mRgpsc = mTgps_from_w.rowRange(0,3).colRange(0,3) * mRwc;
    // mtgpsc = mTgps_from_w.rowRange(0,3).colRange(0,3) * mOw + mTgps_from_w.rowRange(0,3).col(3);
    inline cv::Mat GetGPSPosition(){
        return mTgps_from_w.rowRange(0,3).colRange(0,3) * mOw + mTgps_from_w.rowRange(0,3).col(3);
    }
    ...
    inline void SetGPSMatrix(cv::Mat pose){
        mTgps_from_w = pose.clone();
    }
...
public:
    ...
    cv::Mat mTcgps;  // Camera pose in gps coord.
    cv::Mat mTgpsc;
    cv::Mat mTwc_sensor;
    cv::Mat mTcw_sensor;
    cv::Mat mTgps_from_w; // RT slam world to gps coord.
